...
def generate_report(data_date: str, output_path: str = None) -> str:
    ...
    w("## 一、总体概览\n")
    cross = df.groupby("store_rating").agg(
        数量=("store_id","count"),
    ...
    for r in ["A","B","C"]:
        if r in cross.index:
            x = cross.loc[r]
            w(f"| {r} | {int(x['数量'])} | {int(x['数量'])/total*100:.1f}% | {x['平均评分']:.0f} | {x['平均逾期率']*100:.2f}% | {x['平均交易笔数']:.0f} | {x['平均活跃月数']:.1f} |")
    w("\n> ✅ 趋势正确") if all(cross.loc[r,'平均逾期率'] < cross.loc['C','平均逾期率'] for r in ['A','B'] if r in cross.index) else w("\n> ⚠️ 趋势异常")

    w("\n## 二、渠道等级 × 评分\n")
    cl = df[df["channel_level"].notna()].groupby("channel_level").agg(
    ...
    od = df.groupby("逾期段", observed=True).agg(数量=("store_id","count"), 平均评分=("compliance_score","mean"))
    w("| 逾期率 | 数量 | 平均评分 |")
    w("|:-----:|:----:|:-------:|")
