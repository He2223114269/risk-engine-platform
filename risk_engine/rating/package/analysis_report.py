...
def generate_report(data_date: str, output_path: str = None) -> str:
    ...
    for r in ["A", "B", "C"]:
        if r in cross.index:
            x = cross.loc[r]
            w(
                f"| {r} | {int(x['数量'])} | {int(x['数量'])/total*100:.1f}% | {x['平均评分']:.0f} | {x['平均逾期率']*100:.2f}% | {x['平均退订率']*100:.2f}% | {x['平均办单量']:.0f} |"
            )
    ...
    for _, r in top.iterrows():
        w(
            f"| {r['pack_name'][:40]} | {r['package_rating']} | {r['compliance_score']:.0f} | {r['num_overdue_rate']*100:.2f}% | {int(r['total_transaction_count'])} |"
        )
    ...
    od = df.groupby("逾期段", observed=True).agg(
        数量=("pack_name", "count"), 平均评分=("compliance_score", "mean")
    )
    w("| 逾期率 | 数量 | 平均评分 |")
    w("|:-----:|:----:|:-------:|")
