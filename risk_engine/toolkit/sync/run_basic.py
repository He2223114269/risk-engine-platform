...
lc = pymysql.connect(host='172.31.80.1', port=3306, user='root', password='222311', database='risk_control')
cur = lc.cursor()
B = 10000
...
def create_table(cursor, table_name, columns):
    ...
    sql = "CREATE TABLE `" + table_name + "` (" + ",".join(col_defs) + ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
...
# 2. 风控结果表
sync_table("ods_ods_ts_order_white_list_control", 
           "SELECT * FROM ods.ods_ts_order_white_list_control WHERE type = '淘顺实时授信'")

# 3. 申请表
sync_table("ods_ods_ts_credit_yzf_order_grant_apply",
