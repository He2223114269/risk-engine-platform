...
### 3.1 代理商评级 — supplier v3.1 → v3.2

| 文件 | 状态 | 完成日期 | 备注 |
|------|:----:|:--------:|------|
| `config.py` | ✅ | 05-26→05-28 | 动态权重 + **办单量硬门槛（A≥50单/B≥20单）** |
| `extract.py` | ✅ | 05-26 | StarRocks 取数（含 supplier_name, 过滤空 code） |
| `score.py` | ✅ | 05-26 | 8 维度评分（含企查查：企业正规度+资本实力） |
| `rate.py` | ✅ | 05-26→05-28 | 按省评级 + **apply_min_order_thresholds 办单量过滤** |
| `run.py` | ✅ | 05-26 | 一键运行（含 _merge_qichacha、动态权重重分配） |
| `schema.py` | ✅ | 05-26 | 写入数据契约 |
| `README.md` | ✅ | 05-28 | v3.2 升级（办单量硬门槛说明） |
| `代理商综合评级体系说明.md` | ✅ | 05-28 | 同步更新 v3.2 版本 |

> **v3.2 新增**：办单量硬门槛 — A级近12个月≥50单，B级≥20单，不满足则降级
...
| 05-28 | 代理商品级 v3.1→v3.2 | Phase 3 | 办单量硬门槛（A≥50单/B≥20单），修复高分低量问题 |
